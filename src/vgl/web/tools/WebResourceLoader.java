...
public class WebResourceLoader implements IResourceLoader{
	...
	@Override
	public void loadImage(FileDetails imageFile, Callback<Image> success, Callback<Throwable> error, ILoadOption...options) {
		loadMap.put(imageFile, new TernaryContainer<>(Image.class, Arrays.asList(options), (image, ignored, fail) -> {
			if(fail == null)
	...
	private void loadByClass(FileDetails file,
	                         ...
	                         TernaryCallback<? super IResource, ? super IResource, Throwable> finisher
	                         ) throws Throwable {
		if(type.equals(Image.class)) {
			WebSpecific.FileLoading
			           .loadImage0(file, (image, error) -> {
			        	   if(error != null) {
			        	   ...
			        	   if(options != null) {
		      			    ...
		      			    if(options != null && options.stream().anyMatch(op -> op == StandartLoadOption.LOAD_NEXT_UPDATE)) {
		      			    	ProcessManager.get()
		      			    	              .runNextUpdate(() -> {
		      			    	            	  finisher.invoke(image.getDataFormat() == loadFormatFinal ? image :
		      			    				                      Image.copy(image, loadFormatFinal, true),
		      			    				                      (IResource) null,
		      			    				                      (Throwable) null);
		...
		if(type.equals(Sound.class)) {
			VGL.io
			   .readBytes(file, buffer -> {
				        AudioDecoder.decodeAudio(WebSpecific.JS.cast(buffer),
				             alBuffer -> {
